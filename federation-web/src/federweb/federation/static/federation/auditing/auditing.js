...
$(function () {
    ...
    $.ajax({
        type: "GET",
        url: getProxiedUrl(sprintf("%s/users", fedApiAddress)),
        dataType: "json"
    })
        .done(function (data, textStatus, jqXHR) {
            var options = $('#users-select');
            $.each(data, function (index, item) {
                var matches = /\/users\/([\w-]+)/.exec(item.uri);
                var uuid = matches[1];
                var name = item.username + ' (' + uuid + ')';
                options.append($("<option />").val(uuid).text(name));
            });
        })
...
function requestAuditingEventsReport() {
    ...
    var userFilter = $("#users-select").val();
    if (userFilter) {
        data.filter["user"] = userFilter;
    }
